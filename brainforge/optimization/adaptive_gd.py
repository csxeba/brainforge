...
class Adagrad(_SGD):
    ...
    def initialize(self, nparams, memory=None):
        self.memory = xp.zeros((nparams,)) if memory is None else memory

    def optimize(self, W, gW, m):
        nabla = gW / m
        self.memory += nabla**2
        updates = (self.eta / xp.sqrt(self.memory + self.epsilon)) * nabla
        return W - updates
...
#             self.gmemory = xp.zeros((self.brain.nparams,))
#         if self.umemory is None:
#             self.umemory = xp.zeros((self.brain.nparams,))
#
#     def _opt_coroutine(self, rho, epsilon, gmemory, umemory):
#         update = xp.zeros_like(gmemory)
#         W, gW, m = yield update
...
class RMSprop(Adagrad):
    ...
    def optimize(self, W, gW, m):
        ...
        updates = (self.eta * nabla) / xp.sqrt(self.memory + self.epsilon)
        return W - updates
...
class Adam(_SGD):
    ...
    def optimize(self, W, gW, m):
        ...
        update = (eta * self.velocity) / xp.sqrt(self.memory + self.epsilon)
        return W - update
